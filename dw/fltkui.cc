...
#include <stdio.h>
#include <cstdint> // 또는 #include <stdint.h>
...
{
   return index == (intptr_t) ((Fl_Choice *)widget)->mvalue()->user_data();
}
...
void FltkListResource::widgetCallback (Fl_Widget *widget, void *data)
{
   ...
   int index = -1;
   if (fltkItem)
      index = (intptr_t) (fltkItem->user_data ());
   if (index > -1) {
...
void *FltkListResource::newItem (const char *str, bool enabled, bool selected)
{
   ...
   item->user_data((void*)(intptr_t)index);
